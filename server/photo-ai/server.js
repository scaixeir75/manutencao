...
function createHandler({config,verifyToken,acquire,normalize,provider,timeoutMs=45000}){
  return async(req,res)=>{
    ...
    let release,buffer,image;const chunks=[];const controller=new AbortController();
    ...
    try{
      ...
      if(req.method==='OPTIONS'){res.setHeader('Access-Control-Allow-Methods','POST');res.setHeader('Access-Control-Allow-Headers','Authorization, Content-Type');send(204,{});return;}
    ...
    }catch(error){send(controller.signal.aborted?504:(error.status||503),{error:error.status?error.message:'Análise indisponível.'});}
    finally{clearTimeout(timer);chunks.forEach(c=>c.fill(0));chunks.length=0;buffer?.fill(0);image?.fill(0);if(release)await release().catch(()=>{});}
  };
}
async function main(){
  ...
  const server=http.createServer(createHandler({config,verifyToken,acquire,normalize:(b,m)=>normalizeImage(b,m,sharp),provider:analyze}));
